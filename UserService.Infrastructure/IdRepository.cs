...
namespace UserService.Infrastructure
{
    public class IdRepository : IIdRepository
    {
        ...
        public async Task<List<User>> FindUsersById(List<long> ids)
        {
            if (ids.Count > 400) throw new Exception("ids Count Max");
            List<User> users = new List<User>();
            foreach(var i in ids)
            {
               var user = await userManager.FindByIdAsync(i.ToString());
                if(user is null)
                {
                    user = new User("神秘人");
                }
        ...
        public async Task<SignInResult> ChangePhoneNumAsync(long userId, string phoneNum, string token)
        {
        ...
        public async Task<IdentityResult> ChangePasswordAsync(long userId, string password)
        {
        ...
        public async Task ConfirmPhoneNumberAsync(long id)
        {
        ...
        public async Task UpdatePhoneNumberAsync(long id, string phoneNum)
        {
            var user = await userManager.Users.FirstOrDefaultAsync(u => u.Id == id);
            if (user == null)
            {
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public async Task<IdentityResult> RemoveUserAsync(long id)
        {
            ...
            //就会报错：The instance of entity type 'IdentityUserLogin<long>' cannot be tracked because another instance with the same key value for {'LoginProvider', 'ProviderKey'} is already being tracked.
        ...
        public async Task<(IdentityResult, User?, string? password)> ResetPasswordAsync(long id)
        {
            var user = await FindByIdAsync(id);
            if (user == null)
            {
        ...
        public Task<User?> FindByEmialAsync(string emial)
        {
            return userManager.FindByEmailAsync(emial) ;
        }
    }
}
